...
│   ├── tsconfig.json              # 文档站 TypeScript 配置
│   └── tsconfig.node.json
│
├── packages/
│   ├── ui/                        # UI 组件库（核心包）
...
│   │   │   ├── components/
│   │   │   │   ├── Button/
│   │   │   │   │   ├── Button.tsx / Button.css / Button.stories.tsx / index.ts
│   │   │   │   ├── ChatInterface/
│   │   │   │   │   ├── ChatInterface.tsx / ChatInterface.css / ChatInterface.stories.tsx / index.ts
│   │   │   │   ├── Input/
│   │   │   │   │   ├── Input.tsx / Input.css / Input.stories.tsx / index.ts
│   │   │   │   ├── AIMessage/
│   │   │   │   │   ├── AIMessage.tsx / AIMessage.css / AIMessage.stories.tsx / index.ts
│   │   │   │   ├── AILoading/
│   │   │   │   │   ├── AILoading.tsx / AILoading.css / AILoading.stories.tsx / index.ts
│   │   │   │   ├── AIPromptInput/
│   │   │   │   │   ├── AIPromptInput.tsx / AIPromptInput.css / AIPromptInput.stories.tsx / index.ts
│   │   │   │   ├── Select/
│   │   │   │   │   ├── Select.tsx / Select.css / Select.stories.tsx / index.ts
│   │   │   │   └── Checkbox/
│   │   │   │       ├── Checkbox.tsx / Checkbox.css / Checkbox.stories.tsx / index.ts / README.md
│   │   │   ├── __tests__/          # 包级公共测试（冒烟等）
│   │   │   │   └── smoke.test.tsx
│   │   │   ├── types/              # 公共类型
│   │   │   │   └── polymorphic.ts
│   │   │   └── index.ts            # 组件库导出入口
│   │   ├── jest.config.cjs         # 测试配置
│   │   ├── setupTests.ts           # 测试环境初始化（Jest DOM 等）
│   │   ├── tsconfig.json           # TS 配置（开发）
│   │   ├── tsconfig.build.json     # TS 配置（构建）
│   │   ├── tsconfig.node.json
...
├── scripts/                        # 脚本工具
│   ├── audit.ts                    # 依赖审计与报告
│   └── check-pnpm.js               # 校验 pnpm 版本
│
├── docs/index.html                 # 文档站入口 HTML
├── eslint.config.mjs               # ESLint 配置
├── package.json                    # 根工作区配置与脚本
├── pnpm-workspace.yaml             # pnpm 工作区
├── pnpm-lock.yaml
├── README.md
└── AUDIT.md / 面试难题回答.md       # 其他文档
```
...
# 启动文档网站（开发）
pnpm dev               # 等价于 pnpm run docs
pnpm run docs          # 只启动 docs 子包（@velvet/docs）

# 文档构建/预览
pnpm run docs:build

# 组件开发：Storybook（开发/构建）
pnpm storybook
...
# 仅 UI 组件库（@velvetdesign/ui）
pnpm --filter @velvetdesign/ui run build
pnpm --filter @velvetdesign/ui test
pnpm --filter @velvetdesign/ui test:coverage
...
# 其他工具
pnpm audit            # 运行 scripts/audit.ts
pnpm run check-pnpm   # 校验本地 pnpm 版本

# lint-staged 预览与临时跳过（应急）
...
1. 组件开发：在 `packages/ui/src/components/` 中创建组件目录
2. 故事编写：在各组件目录内编写 `*.stories.tsx`
3. 组件导出：在 `packages/ui/src/index.ts` 中导出组件
4. 组件测试：在 `packages/ui/src/__tests__/` 或组件目录内的 `__tests__` 中编写测试
5. 实时预览：运行 `pnpm storybook`
6. 文档编写：在 `docs/` 中编写使用文档与示例
